...
<div class="w-full">
    ...
    <article class="w-full">
        @if(isset($beritas->gambar) && $beritas->gambar)
            <!-- Featured Image (No cropping) -->
            <div class="w-full mb-8 flex justify-center bg-gray-100 rounded-xl overflow-hidden shadow-sm">
                <img src="{{ asset('storage/' . $beritas->gambar) }}" alt="{{ $beritas->judul }}" class="w-full h-auto max-h-[500px] object-contain">
            </div>
        @else
        ...
        <div class="px-2 md:px-8">
            ...
            <div class="flex flex-wrap items-center gap-4 text-sm text-gray-500 mb-6 border-b border-gray-200 pb-4">
                <span class="flex items-center bg-blue-50 text-blue-700 px-3 py-1 rounded-full font-medium">
                    <i class="far fa-user mr-2"></i> {{ $beritas->penulis ?? 'Admin' }}
                </span>
                <span class="flex items-center">
                    <i class="far fa-calendar-alt mr-2 text-gray-400"></i> {{ \Carbon\Carbon::parse($beritas->created_at)->translatedFormat('d F Y') }}
                </span>
                <span class="flex items-center">
                    <i class="far fa-eye mr-2 text-gray-400"></i> {{ $beritas->views ?? 0 }} kali dilihat
                </span>
            </div>
        ...
        <div class="mt-12 pt-6 px-2 md:px-8 border-t border-gray-200 flex flex-col sm:flex-row justify-between items-center gap-4">
            <p class="text-sm text-gray-500 font-medium">Bagikan berita ini:</p>
            <div class="flex gap-2">
                ...
                <a href="https://twitter.com/intent/tweet?url={{ urlencode(Request::fullUrl()) }}&text={{ urlencode($beritas->judul) }}" target="_blank" class="w-10 h-10 rounded-full bg-blue-400 text-white flex items-center justify-center hover:bg-blue-500 transition-colors shadow-sm">
                    <i class="fab fa-twitter"></i>
                </a>
                <a href="https://api.whatsapp.com/send?text={{ urlencode($beritas->judul . ' - ' . Request::fullUrl()) }}" target="_blank" class="w-10 h-10 rounded-full bg-green-500 text-white flex items-center justify-center hover:bg-green-600 transition-colors shadow-sm">
