        ...
        <div class="row">
            ...
            <div class="col-lg-8 mb-5">
                <div class="mb-8 border-b border-gray-200 pb-4">
                    <h1 class="text-4xl font-bold text-gray-800 border-l-4 border-blue-600 pl-4">Artikel Terbaru</h1>
                    <p class="text-gray-500 mt-2 ml-1">Kumpulan bacaan dan artikel inspiratif dari sekolah.</p>
                </div>
                ...
                <div class="space-y-6">
                    @forelse($artikels as $artikel)
                        <div class="bg-white rounded-xl shadow-sm hover-card overflow-hidden border border-gray-100 flex flex-col sm:flex-row">
                            @if ($artikel->gambar)
                                <div class="sm:w-1/3 h-48 sm:h-auto overflow-hidden relative bg-gray-50 flex items-center justify-center border-r border-gray-100">
                                    <img src="{{ asset('storage/' . $artikel->gambar) }}" alt="Gambar Artikel" class="w-full h-full object-contain p-2 transition duration-500 hover:scale-110" loading="lazy">
                                </div>
                            @else
                                <div class="sm:w-1/3 h-48 sm:h-auto overflow-hidden relative bg-gray-50 flex items-center justify-center border-r border-gray-100">
                                    <img src="{{ asset('foto_artikel.jpg') }}" alt="Gambar Artikel" class="w-full h-full object-cover transition duration-500 hover:scale-110" loading="lazy">
                                </div>
                            @endif
                            <div class="p-5 flex-grow flex flex-col justify-between sm:w-2/3 w-full">
                                <div>
                                    <h3 class="text-xl font-bold text-gray-800 hover:text-blue-600 transition-colors mb-2">
                                        <a href="{{ route('artikel.show', $artikel->id) }}" class="text-decoration-none text-inherit">
                                            {{ $artikel->judul }}
                                        </a>
                                    </h3>
                                    <p class="text-gray-600 text-sm leading-relaxed mb-4">
                                        {{ \Illuminate\Support\Str::limit($artikel->deskripsi, 120) }}
                                    </p>
                                </div>
                                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mt-auto pt-4 border-t border-gray-50 gap-4 sm:gap-0">
                                    <div class="text-xs text-gray-500 flex flex-col sm:flex-row sm:items-center">
                                        <span class="mb-1 sm:mb-0 sm:mr-3"><i class="fas fa-user-edit mr-1"></i> {{ $artikel->penulis }}</span>
                                        <span><i class="far fa-calendar-alt mr-1"></i> {{ \Carbon\Carbon::parse($artikel->created_at)->format('d M Y') }}</span>
                                    </div>
                                    <a href="{{ route('artikel.show', $artikel->id) }}" class="btn btn-sm btn-outline-primary rounded-full px-4 font-semibold hover:bg-blue-600 hover:text-white transition-colors w-full sm:w-auto text-center">
                                        Baca
                                    </a>
                                </div>
                            </div>
                    ...
                    @empty
                        <div class="text-center py-10 bg-gray-50 rounded-lg border border-dashed border-gray-300">
                            <p class="text-gray-500 italic">Belum ada artikel yang diterbitkan.</p>
                        </div>
                    @endforelse
                </div>
