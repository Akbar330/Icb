        ...
        <div class="bg-white p-6 rounded-lg shadow-md mt-6">
            <form action="{{ route('admin.artikel.update', $artikel->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="deskripsi" name="deskripsi" value="{{ old('deskripsi', $artikel->deskripsi) }}"
                           class="w-full mt-2 p-2 border border-gray-300 rounded-md" required>
                    @error('deskripsi')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <textarea id="editor" name="konten" rows="6"
                              class="w-full mt-2 p-2 border border-gray-300 rounded-md">{{ old('konten', $artikel->konten) }}</textarea>
                    @error('konten')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
