...
@section('title', 'Edit Artikel / Berita Eskul')

@section('content')
<div class="px-4 py-6 max-w-4xl mx-auto">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-700">Edit Artikel / Berita Eskul</h1>
            <p class="text-sm text-gray-500 mt-1">Perbarui judul, foto cover, cuplikan, atau konten artikel ekstrakurikuler.</p>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8">
        <form action="{{ route('admin.kegiatan-eskul.update', $kegiatan->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                @if(auth()->user()->role === 'eskul')
                    <input type="hidden" name="eskul_id" value="{{ $kegiatan->eskul_id }}">
                    <input type="text" disabled value="{{ $kegiatan->eskul ? $kegiatan->eskul->nama_eskul : '-' }}"
                ...
                @else
                    <select name="eskul_id" id="eskul_id" required
                        ...
                        @foreach($eskulList as $e)
                            <option value="{{ $e->id }}" {{ old('eskul_id', $kegiatan->eskul_id) == $e->id ? 'selected' : '' }}>{{ $e->nama_eskul }} ({{ $e->kategori }})</option>
                        @endforeach
                    </select>
                @endif
                @error('eskul_id')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="penulis" name="penulis" value="{{ old('penulis', $kegiatan->penulis ?: ($kegiatan->user ? $kegiatan->user->name : '')) }}"
                           class="w-full p-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    @error('penulis')
                        <span class="text-red-500 text-xs">{{ $message }}</span>
                    @enderror
                ...
                <div>
                    ...
                    <input type="date" id="tanggal_kegiatan" name="tanggal_kegiatan" value="{{ old('tanggal_kegiatan', $kegiatan->tanggal_kegiatan ? $kegiatan->tanggal_kegiatan->format('Y-m-d') : '') }}"
                           class="w-full p-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div>
                ...
                <input type="text" id="deskripsi" name="deskripsi" value="{{ old('deskripsi', $kegiatan->deskripsi) }}" required
                       class="w-full p-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                @error('deskripsi')
                    <span class="text-red-500 text-xs">{{ $message }}</span>
                @enderror
            ...
            <div>
                <label for="foto" class="block text-gray-700 font-semibold text-sm mb-1">Gambar Cover Artikel</label>
                @if($kegiatan->foto)
                    <div class="mb-2 flex items-center gap-3 p-2 bg-gray-50 rounded-lg border border-gray-200">
                        <img src="{{ asset('storage/' . $kegiatan->foto) }}" alt="Current Cover" style="height: 60px; width: 100px; object-fit: cover; border-radius: 6px;">
                        <span class="text-xs text-gray-500">Foto cover saat ini. Pilih file baru di bawah hanya jika ingin menggantinya.</span>
                    </div>
                @endif
            ...
            <div>
                ...
                <textarea id="editor" name="konten" rows="12"
                          class="w-full p-2 border border-gray-300 rounded-lg text-sm">{{ old('konten', $kegiatan->konten ?: $kegiatan->deskripsi) }}</textarea>
                @error('konten')
                    <span class="text-red-500 text-xs">{{ $message }}</span>
                @enderror
            </div>
